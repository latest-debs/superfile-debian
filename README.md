![superfile for Debian](.github/readme-header.png)

# superfile for Debian

[yorukot/superfile](https://github.com/yorukot/superfile) — A fast, modern terminal file manager written in Go, with multi-panel navigation, previews, and plugin support. —
...
Or download a `.deb` from the [Releases](https://github.com/latest-debs/superfile-debian/releases) page:

```sh
sudo dpkg -i superfile_*.deb
```
...
- amd64, arm64, armhf, i386, ppc64el, riscv64, s390x — whichever
  architectures yorukot/superfile actually publishes a Linux binary for
...
For issues with superfile itself, see
